...
#Defines the base directory as workspace/ relative to the server script. # This directory represents 
# our roots boundary - the server will only operate within this directory. Creates the directory 
# if it doesn't exist.
BASE_DIR = Path(__file__).parent / "workspace"
BASE_DIR.mkdir(exist_ok=True)


#Security function that checks if a given path is within the allowed roots directory. Uses resolve() 
# to handle symlinks and relative_to() to verify the path is a subdirectory of BASE_DIR. Returns False 
# if path escapes roots boundary.
...
#The is_within_roots() check ensures clients cannot read files outside the workspace directory.
@mcp.tool()
...
# Same roots checking prevents writing outside workspace directory.
@mcp.tool()
...
#(FILE/DIR), relative path, and size for a valid directory not in ROOT List
@mcp.tool()
...
#sampling request to client/LLM  for code analysis
@mcp.tool()
...
#resources are for passive data access (read-only), tools are for active operations (read/write/execute).
